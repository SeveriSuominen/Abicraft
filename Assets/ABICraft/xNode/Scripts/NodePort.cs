...
namespace AbicraftNodeEditor {
    ...
    public class NodePort {
        ...
        public AbicraftNode.ConnectionType connectionType { get { return _connectionType; } }
        public AbicraftNode.TypeConstraint typeConstraint { get { return _typeConstraint; } }
        ...
        public AbicraftNode node { get { return _node; } }
        ...
        [SerializeField] private AbicraftNode.ConnectionType _connectionType;
        [SerializeField] private AbicraftNode.TypeConstraint _typeConstraint;
        ...
        public NodePort(FieldInfo fieldInfo) {
            ...
            for (int i = 0; i < attribs.Length; i++) {
                if (attribs[i] is AbicraftNode.InputAttribute) {
                    _direction = IO.Input;
                    _connectionType = (attribs[i] as AbicraftNode.InputAttribute).connectionType;
                    _typeConstraint = (attribs[i] as AbicraftNode.InputAttribute).typeConstraint;
                } else if (attribs[i] is AbicraftNode.OutputAttribute) {
                    _direction = IO.Output;
                    _connectionType = (attribs[i] as AbicraftNode.OutputAttribute).connectionType;
                    _typeConstraint = (attribs[i] as AbicraftNode.OutputAttribute).typeConstraint;
                }
            }
        }

        /// <summary> Copy a nodePort but assign it to another node. </summary>
        public NodePort(NodePort nodePort, AbicraftNode node) {
        ...
        public NodePort(string fieldName, Type type, IO direction, AbicraftNode.ConnectionType connectionType, AbicraftNode.TypeConstraint typeConstraint, AbicraftNode node) {
...
#endif
            if (port.connectionType == AbicraftNode.ConnectionType.Override && port.ConnectionCount != 0) { port.ClearConnections(); }
            if (connectionType == AbicraftNode.ConnectionType.Override && ConnectionCount != 0) { ClearConnections(); }
        ...
        public bool CanConnectTo(NodePort port) {
            ...
            if (input.typeConstraint == AbicraftNode.TypeConstraint.Inherited && !input.ValueType.IsAssignableFrom(output.ValueType)) return false;
            if (input.typeConstraint == AbicraftNode.TypeConstraint.Strict && input.ValueType != output.ValueType) return false;
            if (input.typeConstraint == AbicraftNode.TypeConstraint.InheritedInverse && !output.ValueType.IsAssignableFrom(input.ValueType)) return false;
            // Check output type constraints
            if (output.typeConstraint == AbicraftNode.TypeConstraint.Inherited && !input.ValueType.IsAssignableFrom(output.ValueType)) return false;
            if (output.typeConstraint == AbicraftNode.TypeConstraint.Strict && input.ValueType != output.ValueType) return false;
            if (output.typeConstraint == AbicraftNode.TypeConstraint.InheritedInverse && !output.ValueType.IsAssignableFrom(input.ValueType)) return false;
            // Success
            return true;
        }
        ...
        public void Redirect(List<AbicraftNode> oldNodes, List<AbicraftNode> newNodes) {
        ...
        private class PortConnection {
            ...
            [SerializeField] public AbicraftNode node;
