...
{
    [CustomNodeEditor(typeof(CastAttributesNode))]
    public class CastAttributesEditor : NodeEditor
    {
        CastAttributesNode node;
        AbicraftObject obj;
        ...
        public override void OnHeaderGUI(GUIStyle style)
        {
            ...
            if (node == null)
            {
                node = target as CastAttributesNode;
            }
        ...
        public override void OnBodyGUI()
        {
            ...
            if (AbicraftGlobalContext.HasValidAbicraftInstance())
            {
                for (int i = 0; i < abicraft.dataFile.GlobalAttributes.Count; i++)
                {
                    ...
                    for (int j = 0; j < attr.effects.Count; j++)
                    {
                        if (attr.effects[j].effect == AbicraftAttribute.AttributeEffect.Effect.OnCast)
                            containsCast = true;
                    }
            ...
            NodeEditorGUILayout.PropertyField(serializedObject.FindProperty("TargetObj"), new GUIContent("Target"));
            ...
            if (node.selectedIndex != node.lastIndex)
            {
                ...
                {
                    if (attrs[node.selectedIndex] != null && !node.allSelectedIndices.Contains(new CastAttributesNode.AttributeCast(attrs[node.selectedIndex])))
                        node.allSelectedIndices.Add(new CastAttributesNode.AttributeCast(attrs[node.selectedIndex]));
