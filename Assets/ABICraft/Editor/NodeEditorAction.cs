...
namespace AbicraftNodeEditor {
    public partial class NodeEditorWindow {
        ...
        public static AbicraftNode[] copyBuffer = null;
                    ...
                    if (e.button == 0) {
                        ...
                        if (currentActivity == NodeActivity.DragNode) {
                            ...
                            for (int i = 0; i < Selection.objects.Length; i++) {
                                if (Selection.objects[i] is AbicraftNode) {
                                    AbicraftNode node = Selection.objects[i] as AbicraftNode;
                    ...
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null) {
                                AbicraftNode node = draggedOutputTarget.node;
                            ...
                            IEnumerable<AbicraftNode> nodes = Selection.objects.Where(x => x is AbicraftNode).Select(x => x as AbicraftNode);
                            foreach (AbicraftNode node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        } else if (!IsHoveringNode) {
        ...
        public void Home() {
            var nodes = Selection.objects.Where(o => o is AbicraftNode).Cast<AbicraftNode>().ToList();
            if (nodes.Count > 0) {
        ...
        public void RemoveSelectedNodes() {
            ...
            foreach (UnityEngine.Object item in Selection.objects) {
                if (item is AbicraftNode) {
                    AbicraftNode node = item as AbicraftNode;
        ...
        public void DuplicateSelectedNodes() {
            ...
            AbicraftNode[] selectedNodes = Selection.objects.Select(x => x as AbicraftNode).Where(x => x != null && x.graph == graph).ToArray();
        ...
        public void CopySelectedNodes() {
            copyBuffer = Selection.objects.Select(x => x as AbicraftNode).Where(x => x != null && x.graph == graph).ToArray();
        }
        ...
        private void InsertDuplicateNodes(AbicraftNode[] nodes, Vector2 topLeft) {
            ...
            Dictionary<AbicraftNode, AbicraftNode> substitutes = new Dictionary<AbicraftNode, AbicraftNode>();
            for (int i = 0; i < nodes.Length; i++) {
                AbicraftNode srcNode = nodes[i];
                if (srcNode == null) continue;
                AbicraftNode newNode = graphEditor.CopyNode(srcNode);
        ...
        bool IsHoveringTitle(AbicraftNode node) {
        ...
        public void AutoConnect(AbicraftNode node) {
